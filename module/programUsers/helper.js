...
module.exports = class ProgramUsersHelper {
  ...
  static create(data) {
    return new Promise(async (resolve, reject) => {
      try {
        let programUsers = await database.models.programUsers.create(data);
  ...
  static checkForUserJoinedProgramAndConsentShared(programId, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let programUsers = await this.programUsersDocuments(query, [
          "_id",
          "consentShared",
        ]);
        result.joinProgram = programUsers.length > 0 ? true : false;
        result.consentShared =
          programUsers.length > 0 ? programUsers[0].consentShared : false;
